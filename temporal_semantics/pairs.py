...
def _feature_norm_delta(a: List[float], b: List[float]) -> float:
    na = math.sqrt(sum(v * v for v in a))
    nb = math.sqrt(sum(v * v for v in b))
    if na == 0:
        return 0.0 if nb == 0 else 1.0
    return abs(nb - na) / na


def _is_tile_compatible(artifact_row: Dict[str, str]) -> bool:
    caps = artifact_row.get("capabilities_json", "")
    if not caps:
        return True
    payload = json.loads(caps)
    return bool(payload.get("tile_compatible", False))
...
def build_temporal_semantic_features(
    ...
    fuse_weights = {
        "semantic_score_lposs": 0.35,
        "semantic_score_dinov2": 0.25,
        "semantic_score_clip": 0.2,
        "semantic_score_siglip2": 0.15,
        "backend_agreement_score": 0.03,
        "backend_disagreement_score": -0.02,
    ...
    for pair in pairs:
        ...
        for backend in requested_backends:
            prev_art = artifact_map.get((prev_id, backend))
            curr_art = artifact_map.get((curr_id, backend))
            if prev_art is None or curr_art is None:
                continue
            if not (_is_tile_compatible(prev_art) and _is_tile_compatible(curr_art)):
                continue
            ...
            for tile in tiles:
                ...
                row = {
                    ...
                    "backend_disagreement_score": 0.0,
                ...
                if backend == "lposs":
                    ...
                    row.update({"mask_change_density": d_mask, "class_histogram_drift": d_hist, "prob_entropy_change": d_entropy})
                    raw = weighted_score({"mask_change_density": d_mask, "class_histogram_drift": d_hist, "prob_entropy_change": d_entropy}, lposs_weights)
                else:
                    v_prev = prev_feat.get(tile_key)
                    v_curr = curr_feat.get(tile_key)
                    if v_prev is None or v_curr is None:
                        d_cos, d_l2, d_norm = 1.0, 1.0, 1.0
                    else:
                        d_cos = feature_cosine_distance(v_prev, v_curr)
                        d_l2 = feature_l2_distance(v_prev, v_curr)
                        d_norm = _feature_norm_delta(v_prev, v_curr)
                    row.update({"feature_cosine_distance": d_cos, "feature_l2_distance": d_l2, "feature_norm_delta": d_norm})
                    raw = weighted_score({"feature_cosine_distance": d_cos, "feature_l2_distance": d_l2}, feature_weights) + 0.1 * d_norm
        ...
        if active_backends:
            for tile in tiles:
                ...
                for backend in active_backends:
                    ...
                    backend_row["backend_disagreement_score"] = disagree
