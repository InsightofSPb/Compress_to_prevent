...
class CLIPBackend(SemanticBackend):
    ...
    def capabilities(cls) -> BackendCapabilities:
        return BackendCapabilities(
            backend=cls.name,
            model_id="openclip_proxy_patch_v1",
            ...
            experimental=False,
            notes="OpenCLIP-style dense patch proxy backend.",
        )
    ...
    def export_artifacts(self, sample: Dict[str, str], out_dir: Path, tile_size: int) -> Dict[str, object]:
        ...
        for y0 in range(0, height, tile_size):
            for x0 in range(0, width, tile_size):
                ...
                n = max(len(luma), 1)
                features.append({"x": x0 // tile_size, "y": y0 // tile_size, "vec": [sum(luma) / n, sum(rg) / n, sum(bg) / n, edge / n]})
