...
class RawCosineScorer(nn.Module):
    def __init__(self, scale: float = 100.0, eps: float = 1e-12):
        super().__init__(); self.scale = float(scale); self.eps = eps

    def forward(self, features: torch.Tensor, prototypes: torch.Tensor,
                *, scale: torch.Tensor | float | None = None,
                bias: torch.Tensor | float | None = None) -> torch.Tensor:
        if features.ndim not in (3, 4): raise ValueError("features must be [D,H,W] or [N,D,H,W]")
        if prototypes.ndim != 2: raise ValueError("prototypes must be [C,D]")
        unbatched = features.ndim == 3
        if unbatched: features = features.unsqueeze(0)
        ...
        if scale.ndim: scale = scale.view(1, -1, 1, 1)
        if bias.ndim: bias = bias.view(1, -1, 1, 1)
