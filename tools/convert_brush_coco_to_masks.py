...
- Supports manual filename overrides (RENAMED) when COCO JSON filenames differ from files on disk.
- IMPORTANT: masks/overlays are saved using the ACTUAL found image filename (on disk),
  i.e. if JSON says "bac294e9-man_7_1.png" but disk has "manezhnyi_7_2025.png",
  the mask will be saved as "manezhnyi_7_2025.png" -> "manezhnyi_7_2025.png" stem + ".png".
...
# Manual overrides for renamed files:
# Key: filename as stored in COCO JSON ("images[].file_name")
# Value: actual filename on disk inside --images-root
RENAMED: Dict[str, str] = {
    "bac294e9-man_7_1.png": "manezhnyi_7_2025.png",
    "abe40060-man_7_2.png": "manezhnyi_7_2024.png",
    "d09794d1-man_7_3.png": "manezhnyi_7_2023.png",
    "6b06009e-ryileeva_26_2.png": "ryileeva_26_2024.png",
    "4bb74ad5-ryileeva_26_3.png": "ryileeva_26_2023.png",
}
...
def find_image_path(images_root: Path, file_name_from_json: str) -> Path | None:
    if file_name_from_json in RENAMED:
        mapped = RENAMED[file_name_from_json]
        candidate = images_root / mapped
        if candidate.exists():
            return candidate
        LOGGER.warning( "Manual rename exists for JSON name %s → %s, but target file not found under images-root.", file_name_from_json, mapped, )

    direct = images_root / file_name_from_json
    if direct.exists():
        return direct
    if "-" in file_name_from_json:
        tail = file_name_from_json.split("-", 1)[1]
        candidate = images_root / tail
        if candidate.exists():
            return candidate
    tail_only = Path(file_name_from_json).name
...
def convert(args: argparse.Namespace) -> None:
    ...
    for img_entry in tqdm(images, desc="Обработка изображений", unit="img", total=len(images)):
        ...
        json_file_name = img_entry["file_name"]
        width, height = img_entry["width"], img_entry["height"]

        img_path = find_image_path(args.images_root, json_file_name)
        if img_path is None:
            LOGGER.warning("Image not found: %s (id=%s)", json_file_name, image_id)
            continue

        image = Image.open(img_path).convert("RGB")

        if image.size != (width, height):
            LOGGER.warning( "Size mismatch for JSON=%s (id=%s): JSON has (%d, %d), image is (%d, %d)", json_file_name, image_id, width, height, image.size[0], image.size[1], )
        ...
        mask_path = args.masks_out / f"{out_stem}.png"
        Image.fromarray(mask, mode="L").save(mask_path)

        if args.overlays_out:
            overlay_path = args.overlays_out / f"{out_stem}_overlay.jpg"
