...
def apply_crop_mask(gray: np.ndarray) -> np.ndarray:
    h, w = gray.shape
    top = int(round(h * TOP_CROP_FRAC))
    bottom = h - int(round(h * BOTTOM_CROP_FRAC))
    left = int(round(w * SIDE_CROP_FRAC))
    right = w - int(round(w * SIDE_CROP_FRAC))
    cropped = gray.copy()
    cropped[:top, :] = 0
    cropped[bottom:, :] = 0
    cropped[:, :left] = 0
...
def find_homography(kp_a, kp_b, matches: Sequence[cv2.DMatch]) -> Tuple[
    Optional[np.ndarray], int, Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]
]:
    if not matches:
    ...
    return H, num_inliers, mask, pts_a, pts_b


def median_reprojection_error(H: np.ndarray, pts_a: np.ndarray, pts_b: np.ndarray, mask: Optional[np.ndarray]) -> float:
    if H is None or pts_a is None or pts_b is None or mask is None:
        return 0.0
    inliers = mask.ravel() > 0
    if not np.any(inliers):
        return 0.0
    projected = cv2.perspectiveTransform(pts_a, H)
    errors = np.linalg.norm(projected[inliers] - pts_b[inliers], axis=2).ravel()
    if errors.size == 0:
        return 0.0
...
def process_pair(entry: PairEntry, out_dir: Path, detector_method: str) -> DebugResult:
    try:
        ...
        mask_a_for_features = apply_crop_mask(mask_a_gray)
        mask_b_for_features = apply_crop_mask(mask_b_gray)
    except Exception:
        return DebugResult(
    ...
    edge_a = compute_edges(mask_a_for_features)
    edge_b = compute_edges(mask_b_for_features)
    ...
    H, num_inliers, inlier_mask, pts_a, pts_b = find_homography(kp_a, kp_b, matches)
    inlier_ratio = num_inliers / num_matches if num_matches > 0 else 0.0
    median_err = median_reprojection_error(H, pts_a, pts_b, inlier_mask)
    ...
    if H is None or num_inliers < 20:
        ...
        return DebugResult(
            ...
            reproj_error_median=median_err,
            iou_fg=0.0,
            iou_edge=0.0,
            overlay_path=overlay_path,
        )
            ...
            writer.writerow(
                {
                    ...
                    "reproj_error_median": f"{res.reproj_error_median:.6f}",
